...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        if ((pid = fork()) == 0)
        {
            dbg_client_cpid("New client process");
            ...
            dbg_client_cpid("Size of recieved packet: %d", packet_bytes);
            if (auth_match(auths, client.user, client.pass) == 0)
            { // all good, send good response and continue
                dbg_client_cpid("Valid client details");
                dbg_client_cpid("Sending authentication approval");
                if (send(new_fd, &yes, sizeof(yes), 0) == -1)
                {
                    log_err("send good auth");
                    exit(1);
                }
                dbg_client_cpid("Size of sent packet: %d", (int)sizeof(yes));
            } else
            { // not good, send bad response and close socket
                msg_client_cpid("Invalid client details");
                dbg_client_cpid("Sending authentication disaproval");
                if (send(new_fd, &no, sizeof(no), 0) == -1)
                { // client knows '1' is bad
                    log_err("send bad auth");
                }
                dbg_client_cpid("Size of sent packet: %d", (int)sizeof(no));
                msg_server("Closing connection from %s", inet_ntoa(their_addr.sin_addr));
                close(new_fd);
                exit(0);
            ...
            while(1)
            {
                ...
                dbg_client_cpid("Size of recieved packet: %d", packet_bytes);
                dbg_client_cpid("Player name recieved: %s", input);
                ...
                { // player name not found
                    dbg_client_cpid("Player '%s' was not found", input);
                    if (send(new_fd, &no, sizeof(no), 0) == -1)
                ...
                { // player name found
                    dbg_client_cpid("Player '%s' was found", player->name);
                    ...
                    dbg_client_cpid("Size of sent packet: %d", (int)sizeof(*player));
                }
            }
            //
            // --------------------------------------
